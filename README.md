...
SSH (22) → 0.0.0.0/0 (temporary for lab)
...
✔ Confirms EC2 is using IAM role
...
✔ Pure role-based access (best practice)
...
🎯 Objective (What Part-2 Achieves)
...
Path:
Launch Instance → Resource tags → Add tag
...
🎯 Objective (What Part-3 Achieves)
...
Path:
Lambda → basic-hello-lambda → Monitor → View logs in CloudWatch

You will see:

✔ Log Group
/aws/lambda/basic-hello-lambda
...
processed_test.txt → ✅ created automatically
...
✅ DEVOPS LAB — SECTION-6
Lambda to Move Files Between Folders in S3
...
incoming/testfile.txt ❌ removed

processed/testfile.txt ✅ appears automatically
...
IAM Role → temporary credentials (BEST PRACTICE for EC2)

2️⃣ Trust Policy vs Permission Policy
Policy Type	Purpose
...
🏗 Architecture Overview
EC2-A ──(TrainingBucketRole)──► training-bucket-a
        ❌ project-bucket-b

EC2-B ──(ProjectBucketRole)──► project-bucket-b
        ❌ training-bucket-a
...
Policy name: ProjectBucketPolicy
...
aws s3 ls s3://project-bucket-b-<unique>     # ❌ AccessDenied

EC2-B
aws s3 ls s3://project-bucket-b-<unique>     # ✅ Allowed
